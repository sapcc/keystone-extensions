...
class Password(base.AuthMethodHandler):
    ...
    def authenticate(self, request, auth_payload):
        ...
        try:
            ...
            LOG.info("Authenticating %s@%s.." % (auth_payload['user'].get('name', auth_payload['user'].get('id', '<missing>')), domain.get('name', domain.get('id', '<missing>'))))
        ...
        try:
            ...
            try:
                # check if the user actually exists in domain, since the exception doesn't tell anything about the root-cause
                u = self.identity_api.get_user(user_info.user_id)
            except Exception as e:
                LOG.info("Authentication failed: %s" % e)
    ...
    def _authenticate_ews(self, username, password):
    ...
    def _update_password(self, userinfo):
        """Update a users password in LDAP"""
        try:
            LOG.info("Updating password of %s" % userinfo.user_ref['name'])
            # CC special hack: add a fake login attribute, to allow initial user onboarding of freshly provisioned CAM users
