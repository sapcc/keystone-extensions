...
class Base(base.AuthMethodHandler):
    def authenticate(self, request, auth_info):
        ...
        try:
            ...
            verification = request.environ[CONF.cc_x509.certificate_verify_header]
            if verification != 'SUCCESS':
                raise Exception("Certificate has not been validated")

            # grab the certificate
            certificate = request.environ[CONF.cc_x509.certificate_header]
            cert = crypto.load_certificate(crypto.FILETYPE_PEM, urlparse.unquote(certificate))
            ...
            # check the issuer
            issuer =  ",".join("%s=%s" % tup for tup in reversed(cert.get_issuer().get_components()))
            ...
            LOG.info("Authentication failed. Invalid certificate from %s: %s" % (request.environ['REMOTE_ADDR'], e));
            if CONF.debug:
        ...
        try:
            user_ref = self._authenticate(username, request)
            ...
            LOG.info(
                "Authentication failed. Invalid username %s from %s: %s" % (username, request.environ['REMOTE_ADDR'], e))
...
class Certificate(Base):
    ...
    def _authenticate(self, username, request):
        """
        ...
        domain_name = None
        try:
            domain_id = request.environ[CONF.cc_x509.user_domain_id_header]
        except KeyError:
            try:
                domain_name = request.environ[CONF.cc_x509.user_domain_name_header]
            except KeyError:
                LOG.error("Authentication failed. Missing domain header from: %s" % request.environ['REMOTE_ADDR'])
